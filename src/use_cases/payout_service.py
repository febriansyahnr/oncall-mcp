from src.infrastructure.database import get_db_connection, payout_table
from src.domain.models import PayoutTransactionRead
...
def find_transaction_by_uuid(uuid: str) -> PayoutTransactionRead | None:
    ...
    with get_db_connection('xb') as conn:
        stmt = select(payout_table).where(payout_table.c.uuid == uuid)
...
def find_transaction_by_client_id(client_transaction_id: str) -> PayoutTransactionRead | None:
    ...
    with get_db_connection('xb') as conn:
        stmt = select(payout_table).where(payout_table.c.client_transaction_id == client_transaction_id)
        result = conn.execute(stmt).fetchone()
...
def list_payout_transactions(limit: int = 10) -> list[PayoutTransactionRead]:
    ...
    with get_db_connection('xb') as conn:
        stmt = select(payout_table).order_by(payout_table.c.created_at.desc()).limit(limit)
